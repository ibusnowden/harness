...
var builtinSpecs = []Spec{
	...
	{Name: "fuzz", Summary: "Run defensive fuzzing against repo-local targets", Usage: "ascaris fuzz [--scope path] [--budget 2s]", SourceHint: "built-in", Responsibility: "Run the defensive fuzz workflow", ResumeSupported: false},
	{Name: "crash-triage", Summary: "Triage local crash inputs for an authorized binary target", Usage: "ascaris crash-triage --target-cmd /path/to/binary --corpus corpus/", SourceHint: "built-in", Responsibility: "Run the local crash triage workflow", ResumeSupported: false},

#!/usr/bin/env bash
# Print the active OpenAI-compatible base URL for a model served by the RTX fallback job.
#
# Usage:
#   ./inference/endpoint-rtx.sh <model-alias>
#   ./inference/endpoint-rtx.sh --local <model-alias>
#   ./inference/endpoint-rtx.sh --export <model-alias>
set -euo pipefail
...
SLURM_JOB_NAME="${VLLM_SLURM_JOB_NAME:-serve-vllm-rtx}"
METADATA_FILE="${SCRIPT_DIR}/logs/current-${ALIAS}-rtx.env"
...
host_from_metadata() {
    ...
    if command -v squeue >/dev/null 2>&1 && [ -n "${meta_job}" ] && [ "${meta_job}" != "manual" ]; then
        ...
        if [ -n "${node_spec}" ] && [ "${node_spec}" != "(null)" ]; then
            if command -v scontrol >/dev/null 2>&1; then
                local expanded_host=""
                expanded_host="$(scontrol show hostnames "${node_spec}" 2>/dev/null | awk 'NF { print $1; exit }')"
                if [ -n "${expanded_host}" ]; then
                    printf '%s\n' "${expanded_host}"
                    return 0
                fi
            else
